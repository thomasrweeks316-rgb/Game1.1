...
func _populate_lists() -> void:
	_chassis_list.fixed_icon_size = Vector2i(40, 40)
	_chassis_list.clear()
	...
	_weapon_list.fixed_icon_size = Vector2i(40, 40)
	_weapon_list.clear()
	for wid in GameData.WEAPONS:
		if AccountManager.owns_weapon(wid):
			var w := GameData.get_weapon(wid)
			_weapon_list.add_item("%s (DMG:%d)" % [w.get("name", wid), w.get("damage", 0)])
			var idx := _weapon_list.item_count - 1
			_weapon_list.set_item_metadata(idx, wid)
			_weapon_list.set_item_icon(idx, BotArt.get_weapon_icon(wid))
...
func _update_preview() -> void:
	if _preview.has_method("set_loadout"):
		_preview.set_loadout(_selected_chassis, _selected_weapons)
	elif _preview.has_method("set_chassis"):
