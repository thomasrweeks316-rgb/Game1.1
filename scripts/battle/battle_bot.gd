...
func _ready() -> void:
	...
	for child in get_children():
		if child is CollisionShape2D:
			...
			if circle:
				circle.radius = maxf(28.0 * bot_size, _visual_size * 0.85)
...
func _fire_weapon(wid: String, target_pos: Vector2) -> void:
	...
	if wdata.is_empty():
		return
	_fire_cooldowns[wid] = wdata.get("fire_rate", 0.5)
	var dmg := float(wdata.get("damage", 0)) * damage_mult
	match str(wdata.get("type", "bullet")):
...
func _melee_attack(wid: String, wdata: Dictionary, target_pos: Vector2) -> void:
	var attack_range: float = float(wdata.get("range", 70))
	if _target and _target.is_alive() and global_position.distance_to(_target.global_position) <= attack_range:
		var dmg := float(wdata.get("damage", 0)) * damage_mult
...
func _flame_attack(wdata: Dictionary, target_pos: Vector2) -> void:
	...
	var attack_range: float = float(wdata.get("range", 140))
	if dir.dot(to_target) > 0.5 and global_position.distance_to(_target.global_position) < attack_range:
		_target.take_damage(float(wdata.get("damage", 0)) * damage_mult)
...
func _chain_attack(wdata: Dictionary, target_pos: Vector2) -> void:
	...
	for _i in chain_count:
		...
		for node in _battle_scene.get_children():
			if node is BattleBot and node != self and node not in hit and node.is_alive():
		...
		if nearest:
			hit.append(nearest)
			nearest.take_damage(float(wdata.get("damage", 0)) * damage_mult * 0.7)
...
func _gravity_attack(wdata: Dictionary, target_pos: Vector2) -> void:
	var radius: float = float(wdata.get("pull_radius", 100))
	for node in _battle_scene.get_children():
		if node is BattleBot and node != self and node.is_alive():
			var bot := node as BattleBot
			var dist := global_position.distance_to(bot.global_position)
			if dist < radius:
				var pull: Vector2 = (global_position - bot.global_position).normalized() * 200.0
				bot.velocity += pull * 0.05
				bot.take_damage(float(wdata.get("damage", 0)) * damage_mult * 0.1)
