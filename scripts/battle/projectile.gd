...
func setup(bot: BattleBot, wid: String, target_pos: Vector2, dmg: float, wdata: Dictionary, forced_dir: Vector2 = Vector2.ZERO) -> void:
	...
	weapon_data = wdata
	damage = dmg
	max_range = float(wdata.get("range", 200))
	speed = float(wdata.get("projectile_speed", 300))
	if forced_dir != Vector2.ZERO:
		direction = forced_dir.normalized()
	else:
		var aim_delta := target_pos - bot.global_position
		if aim_delta.length_squared() < 0.001:
			direction = Vector2.RIGHT
		else:
			direction = aim_delta.normalized()
...
func _physics_process(delta: float) -> void:
	...
	if _lifetime <= 0:
		queue_free()
		return
	if str(weapon_data.get("type", "")) == "homing" and owner_bot and owner_bot._target:
		...
		rotation = direction.angle()
	if str(weapon_data.get("type", "")) == "mortar":
	...
	if is_boomerang:
		...
		if str(weapon_data.get("type", "")) in ["explosive", "mortar", "vortex"]:
			_explode()
		queue_free()
...
func _on_body_entered(body: Node2D) -> void:
	...
	if body is BattleBot:
		...
		if weapon_type not in ["piercing"]:
			if weapon_type in ["explosive", "mortar"]:
				_explode()
			queue_free()
...
func _hit_bot(bot: BattleBot) -> void:
	var weapon_type := str(weapon_data.get("type", ""))
	var bypass := weapon_type == "piercing"
...
func _explode() -> void:
	...
	for node in parent.get_children():
		if node is BattleBot and node != owner_bot and node.is_alive():
